...
def clean_output_dir(dir_path):
    """清除 OUTPUT_DIR 下的生成文件，但保留 .git 目录"""
...
OUTPUT_DIR = ROOT_DIR.parent               # 仓库根目录（GitHub Pages 部署目录）
...
def load_episodes(shows_by_slug):
    ...
    for show_dir in sorted(CONTENT_DIR.iterdir()):
        ...
        if show_slug not in shows_by_slug:
            print(f"[WARN] content/{show_slug}/ 在 shows.yaml 里找不到对应节目，已跳过")
            continue
...
def main():
    ...
    print(f"[OK] 生成完成：{stats['show_count']} 档节目，{stats['episode_count']} 篇文字稿")
